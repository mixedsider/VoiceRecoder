...
public class WhisperUtil {
    ...
    public String getWordFromToken(int token) {
        byte[] bytes = vocab.tokenToBytes.get(token);
        if (bytes != null) {
    ...
    public String decodeTokenSequence(List<Integer> tokens) {
    ...
    public String decodePieces(List<String> pieces) {
        ...
        for (String word : pieces) {
            ...
            if (isRawBytePiece(word)) {
                byte[] raw = word.getBytes(StandardCharsets.ISO_8859_1);
    ...
    public boolean loadFiltersAndVocab(boolean multilingual, String vocabPath) throws IOException {
        ...
        for (int i = 0; i < nVocab; i++) {
            ...
            vocab.tokenToBytes.put(i, wordBytes);
        }
    ...
    private boolean isRawBytePiece(String word) {
        for (int i = 0; i < word.length(); i++) {
            if (word.charAt(i) > 0x00FF) {
                return false;
            }
        }
        return true;
    ...
    private String decodeUtf8(byte[] bytes) {
        try {
            ...
            Log.w(TAG, "Failed to decode Whisper token bytes as UTF-8", e);
    ...
    public float[] getMelSpectrogram(float[] samples, int nSamples, int nThreads) {
        ...
        for (int workerIndex = 0; workerIndex < nThreads; workerIndex++) {
            final int threadIndex = workerIndex;
            Thread thread = new Thread(() -> {
                float[] fftIn = new float[fftSize];
                Arrays.fill(fftIn, 0.0f);
                float[] fftOut = new float[fftSize * 2];

                for (int i = threadIndex; i < mel.nLen; i += nThreads) {
    ...
    private void fft(float[] input, float[] output) {
        ...
        for (int i = 0; i < inSize; i++) {
            if (i % 2 == 0) {
                even[indexEven++] = input[i];
            } else {
                odd[indexOdd++] = input[i];
            }
        }
    ...
    private static class WhisperVocab {
        ...
        Map<Integer, byte[]> tokenToBytes = new HashMap<>();
