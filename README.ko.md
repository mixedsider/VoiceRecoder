...
VoiceRecoder는 온디바이스 Android 음성 기록 앱입니다. 음성을 녹음하고, 로컬 STT 모델로 텍스트를 변환한 뒤, 로컬 LLM 모델로 녹음별 요약과 일일 요약을 생성합니다.

클라우드 중심 녹음 앱과 달리 VoiceRecoder는 전체 흐름을 로컬에서 해결하도록 설계되었습니다. 필요한 모델을 한 번 다운로드한 뒤에는 녹음, 텍스트 변환, 요약을 인터넷 연결 없이 실행할 수 있으며, 음성 데이터는 기기 안에 유지됩니다.
...
5. 녹음 버튼을 누릅니다. VoiceRecoder는 음성이 들릴 때까지 기다린 뒤 60초 동안 녹음하고, 설정된 정책에 따라 STT와 요약 작업을 큐에 넣습니다.

## 권한

VoiceRecoder는 시작 시 필요한 런타임 권한을 한 번에 요청합니다.
...
VoiceRecoder는 사용자가 요청한 backend와 실제 적용된 backend를 분리해서 관리합니다.
